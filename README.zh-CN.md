...
| 限制 | 今天的影响 | 下一步 |
|---|---|---|
| 磁盘临时 | 结果、历史、生成的 BL 副本和人工补充的文件都无法在重启或重新部署后存活 | 设计好的下一步是把这四样一起做成经过校验的快照存入私有对象存储，新实例必须先恢复并校验通过才对外服务。这一步**只做了设计，没有实现**，见未来路线。托管 Postgres 排在它之后。 |
...
**让状态挺过一次重启。** 现在结果和历史只写在 Render 的临时磁盘上，重启或重新部署即失。这里有
两步，已经设计好但**没有实现**。第一步是整个工作状态的快照——结果存储、生成的 BL 副本、人工补充的
文件——一致地冻结下来、上传到私有对象存储，并且在新实例被允许恢复并对外服务之前**逐个文件校验**；
再用一个条件写更新的指针对象，防止旧实例把更新的状态覆盖掉。第二步才是托管 Postgres，那才是历史
记录、审计轨迹和人工结论真正该待的地方，也是上面那套权限系统存用户和角色的地方。两步都**有意不放进
预赛版本**：它们会改变服务的启动方式，而一个做了一半的版本，比一个诚实写出来的限制更糟。
