...
思考深度的模型配置，第二套只在第一套拒绝下结论时才调用，于是成本花在真正需要的那约 6% 的案子上
（44 次资深调用对 740 次主判），而不是全部 520 封。
...
免费档是预赛原型阶段的一个主动选择，它带来三处可见影响。下面连同各自需要的具体改动一起列出。

| 限制 | 今天的影响 | 下一步 |
|---|---|---|
| 磁盘临时 | 分析结果无法在重启或重新部署后存活 | 整个存储就是一个原子写入的 `results.json`。在 flush 时把它快照到对象存储、启动时取回，只需要一个凭据，**不需要改数据结构**；托管 Postgres 要等到需要隔离多个团队时才划算。 |
| 512 MB 内存 | 同时解析多份 PDF 时 `BATCH_CONCURRENCY` 固定为 4 | 同一套管线、同样 520 封，在更大的机器上以并发 8 跑完只用 10 分 6 秒。提高吞吐靠付费实例和更宽的供应商限速档，不是改应用代码。 |
| 15 分钟休眠 | 冷启动看起来像原型坏了 | 目前用 uptime 监控规避；付费实例可彻底消除。 |
...
这 20 个转人工案件在四种允许的原因上**平均分布**：`wrong_doc_type` 5、`missing_attachment` 5、
...
我们从未打开过主办方的标准答案，所以 5 / 5 / 5 / 5 的均匀分布并不等于正确性证明 —— 但它正是一个
刻意构造的边界案例集应有的形状，而当初正是那个歪掉的分布让我们发现了这个 bug。

**吞吐与并发。** 耗时取决于模型调用的延迟，不取决于我们的代码。16 分 24 秒里发生了 784 次模型调用，
约每秒 0.8 次，后端绝大部分时间都在等供应商返回。`BATCH_CONCURRENCY` 是唯一的调节位：同一套管线、
同样这 520 封邮件，在开发机上以并发 8 跑完只用了 10 分 6 秒。之所以在 Render 免费实例上固定为 4，
是为了在同时解析多份 PDF 时留在 512 MB 内存预算之内。换成付费实例、或换一个限速更宽的供应商档位，
不改任何应用代码就能提高吞吐。
