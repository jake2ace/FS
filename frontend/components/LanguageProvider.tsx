...
const ZH: Record<string, string> = {
  ...
  '← Previous': '← 上一封',
  'Next →': '下一封 →',
  'Nothing is waiting for a decision.': '没有需要你决定的案件。',
  'Use ← and → to move through the queue.': '用 ← 和 → 键在队列里前后切换。',
  'First in the queue': '队列中的第一封',
  'Last in the queue': '队列中的最后一封',
  'Batch Run': '批量运行',
  ...
  Mismatch: '不一致',
  Mismatches: '不一致',
  'at least one of the seven fields differs': '七个字段中至少有一项不同',
  'Needs review': '需要复核',
  'Safe completed': '安全完成',
  'Pending approval': '等待批准',
  'Awaiting draft BL': '等待草稿 BL',
  'Draft BL requested': '待发送草稿 BL',
  'Read the numbers': '先看数字',
  'what the shared inbox needs right now': '共享收件箱现在需要处理什么',
  'Open a case': '打开一个案件',
  'see the seven fields side by side, with the source quote': '七个字段左右对照，每个值都附原文引证',
  'Decide or hand over': '下结论或转人工',
  'confirm it, or let a person take it': '确认处理，或交给人来判断',
  'See a real case →': '看一个真实案例 →',
  'Hide this guide': '隐藏这个引导',
  'Where the work landed': '这些工作最后去了哪里',
  'classified only - no documents to compare': '只做了分类，没有可比对的文件',
  'no mismatch to report': '没有需要报告的差异',
  'handed to a person, with the reason': '带着原因转给了人处理',
  'a field differs between SI and draft BL': 'SI 与草稿 BL 之间有字段不一致',
  'could not be processed at all': '完全无法处理',
  'asked for a Shipping Instruction and a draft Bill of Lading to be checked against each other - those are the only ones that reach the document comparison. This is what happened to them.': '要求把 Shipping Instruction 和草稿提单互相核对——只有这些邮件会进入文件比对环节。下面是它们的去向。',
  'The rest were classified and closed - SI requests, invoice queries, operational notices and spam. They carry no documents to compare.': '其余邮件只做分类后结束——SI 请求、发票询问、运营通知和垃圾邮件，它们没有可比对的文件。',
  'Run the analysis': '先跑一次分析',
  'the system reads every email and both attachments': '系统会读完每封邮件和两个附件',
  'See what needs attention': '看哪些需要处理',
  'it sorts the inbox by how urgent each case is': '系统按紧急程度把收件箱排好序',
  'You decide': '由你来决定',
  'the system never acts on a case by itself': '系统绝不自行处置任何案件',
  'Your work is already sorted': '要处理的已经排好序了',
  'most urgent first - start at the top': '最紧急的排在最前，从第一条开始',
  'Open a case to check it': '打开一个案件核对',
  'every reading quotes the line it came from': '每个读数都附上了它出自原文的哪一行',
  'You decide what happens': '怎么处理由你决定',
  'confirm, hand it on, or send it back': '确认、转交他人，或退回重办',
  'Start with the most urgent →': '从最紧急的开始 →',
  'cases are waiting on you': '个案件在等你决定',
  'Listed most urgent first. The system decided nothing on its own.': '按紧急程度排序。系统没有替你做任何决定。',
  'Mismatch found': '发现字段不一致',
  'contact the carrier and correct the draft': '联系船公司修改草稿提单',
  'The system would not decide': '系统不下结论',
  'read the evidence and make the call': '看证据，由你判断',
  'send the document, then it can be compared': '把文件发出，之后才能比对',
  'Nothing is waiting on a person right now.': '目前没有需要人处理的案件。',
  'The remaining emails in the inbox were classified only - SI requests, invoice queries, notices and spam. They carry no documents to compare.': '收件箱里其余邮件只做了分类——SI 请求、发票询问、通知和垃圾邮件，它们没有可比对的文件。',
  'comparison requests': '封核对请求',
  'need a person': '需要人处理',
  'cases waiting on a person': '个案件在等人处理',
  'to correct': '待联系改单',
  'to decide': '待人工判断',
  'to send': '待发出文件',
  'Open the review queue': '打开复核队列',
  'all seven fields agreed - closed automatically': '七个字段全部一致，已自动完结',
  'the system would not decide - a person must': '系统不下结论，必须由人判断',
  'we have to send the document out first': '需我方先把文件发出',
  'Action list - draft BL requested': '待办清单——待发送草稿 BL',
  'These emails ask our team to send out a draft BL so the customer can check it. No document is attached yet, so there is nothing for the AI to compare and nothing for a reviewer to decide - the work is to send the document, which is why they stay out of the review queue.': '这些邮件要求我方发出草稿提单供客户核对。信里没有附件，因此 AI 没有可比对的内容，复核人员也没有需要判断的事——要做的事是把文件发出去，所以它们不进复核队列。',
  'sender must send the document first': '需对方先发送文件',
  'Action list - waiting for the draft BL': '待办清单——等待草稿 BL',
  emails: '封邮件',
  Request: '请求内容',
  Action: '应采取的动作',
  'These emails ask someone to send a draft BL so it can be checked later. No document has arrived yet, so there is nothing for the AI to compare and nothing for a reviewer to decide - they are ordinary work for a person, kept out of the review queue on purpose.': '这些邮件是请对方发送草稿 BL 以便日后核对。文件尚未送达，因此 AI 没有可比对的内容，复核人员也没有需要判断的事——它们是正常的人工业务动作，因此特意不放进复核队列。',
  'Not analysed': '尚未分析',
